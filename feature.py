...
def ApiExtractor(smalifile:str,categorybasis:dict):
    ...
    Apicnt={"io":0,"net":0,"device":0,"location":0,"UI":0,"hardware":0,"app":0,"multimedia":0,"security":0,"base":0,"floor":0}
...
def GetCategoryBasis():
    ...
    #两个特殊的类android.manifest类属于security；而android.R属于UI
    """
    io\net\device\location\UI\hardware\app\multimedia\security\database\util\system\xml\time
    """
    categorybasis={
         "io":[]\
        ,"net":[]\
    ...
    with open(workplace+"\categorybasis.txt","r") as f:
        for line in f.readlines(): 
            line=line.strip()
            if line and ":" in line:
