...
def DBSCANCluster(npdata):
    clustering = DBSCAN(eps=0.0001,min_samples=30).fit(npdata)
    print(clustering.labels_)

    return clustering.labels_
# for floor in range(1,6):
data,labels,datadic=GetData("D:\workplace\main\libdetector\\feature",3)
...
with open("D:\workplace\main\libdetector\\result\HC\\"+str(3)+".txt","w") as f:
    f.write(str(tmplist))
    f.write("\n\n")
